...
// import CozyPenguinAvax

contract CCIPavax is CCIPReceiver, Withdraw, IERC721Receiver {
    ...
    // CozyPenguinAVAX cozyPenguinAvax;
    address receiver;
    address admin;
    ...
    constructor(
        ...
        admin = _admin;
        // cozyPenguinAvax = CozyPenguinAVAX(_cozyPenguinNft);
    }

    modifier onlyNftOwner(uint256[] calldata _tokenIds) {
        // for (uint256 i = 0; i <= _tokenIds.length; i++) {
        //     uint256 tokenId = _tokenIds[i];
        //     if (cozyPenguinAvax.ownerOf(tokenId) != msg.sender) {
    ...
    /** Locks the existing NFTs on Avalanche.
        ...
        returns (bytes32 messageId)
    {
        ...
        // for (uint256 i = 0; i <= _tokenIds.length; i++) {
        //     uint256 tokenId = _tokenIds[i];
        //    cozyPenguinAvax.safeTransferFrom(msg.sender, address(this), tokenId);
        // }
        ...
        onlyConfirmedSourceChain(message.sourceChainSelector)
    {
        ...
        unlockOrMintPenguin(owner, tokenIds);

        emit MessageReceived(message.messageId);
    }

    /** Unlocks the existing NFTs on Avalanche or mints them if they were not minted yet.
     * Only the contract itself can call it.
     */
    function unlockOrMintPenguin(
        address _owner,
        uint256[] memory _tokenIds
    ) internal {
        for (uint256 i = 0; i <= _tokenIds.length; i++) {
            uint256 tokenId = _tokenIds[i];
            //check if tokenId exists on contract, transfer back if it exists
            // if (cozyPenguinAvax.ownerOf(tokenId) == address(this)) {
            //     cozyPenguinAvax.safeTransferFrom(
            //         address(this),
            //         _owner,
            //         tokenId
            //     );
            // } else {
            //     // mint if tokenId doesn't exist yet
            //     cozyPenguinAvax._safeMint(_owner, tokenId);
            // }
        }
    }
